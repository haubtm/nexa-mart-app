...
import { useMyAddressList, useMyAddressUpdate } from '@/react-query/hooks';
...
export default function EditAddressScreen() {
  ...
  const addressId = id ? Number(id) : 0;
  const { data, isLoading } = useMyAddressList();
  const { mutate: updateAddress, isPending } = useMyAddressUpdate();

  const address = useMemo(() => {
    return data?.data?.find((item) => item.addressId === addressId);
  }, [data, addressId]);
  ...
  const handleSubmit = () => {
    ...
    updateAddress(
      ...
      {
        onSuccess: () => {
          showToast.success('Thành công', 'Đã cập nhật địa chỉ');
          router.back();
        },
        onError: (error: any) => {
          showToast.error('Lỗi', error?.message || 'Cập nhật địa chỉ thất bại');
        },
      },
    );
  };
